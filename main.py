...
from config import TOKEN, CURS_KEY
...
def transform(num_stirng):
    try:
        number = float(num_stirng)
        return number
    except:
        return 0.0
...
async def send_start(message: Message):
   await message.answer(f"Привет, {message.from_user.first_name}! Я Ваш личный финансовый помощник. Выберите одну из опций в меню:", reply_markup=keyboards)
...
async def exchange_rates(message: Message):
   url = f"https://v6.exchangerate-api.com/v6/{CURS_KEY}/latest/USD"
   try:
...
async def finances(message: Message, state: FSMContext):
   await state.update_data(expenses1 = transform(message.text))
...
async def finances(message: Message, state: FSMContext):
   await state.update_data(expenses2 = transform(message.text))
...
async def finances(message: Message, state: FSMContext):
   ...
   cursor.execute('''UPDATE users SET category1 = ?, expenses1 = ?, category2 = ?, expenses2 = ?, category3 = ?, expenses3 = ? WHERE telegram_id = ?''',
                  (data['category1'], data['expenses1'], data['category2'], data['expenses2'], data['category3'], transform(message.text), telegarm_id))
   conn.commit()
   await state.clear()
